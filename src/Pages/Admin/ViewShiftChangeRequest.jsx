...
export default function ViewLeaveRequest() {
    ...
    const fetchRequests = async (username, role) => {
        ...
        try {
            if (role === "admin") {
                const res = await api.get("/admin/shft-change-request");
                ...
                const res = await api.get(`/admin/shft-change-request/${username}`);
    ...
    const handleAction = async (req, action) => {
        try {
            ...
            if (action === "APPROVED") {
                const res = await api.put("/admin/shft-change-request/approve", req);
                console.log(req);
                toast.success(res.data.message);
            } else if (action === "REJECTED") {
                const res = await api.put("/admin/shft-change-request/reject", req);
                toast.success(res.data.message);
            }
        ...
        <>
            ...
            <div className="lr-root">
                ...
                <div className="lr-header mt-10">
                    ...
                    <div className="lr-title">Shift Change <span>Requests</span></div>
                </div>
                    ...
                    <table className="lr-table">
                        <thead className="lr-thead">
                            <tr>
                                ...
                                <th className="lr-th">Date</th>
                                <th className="lr-th">preferredShift</th>
                                    ...
                                    <tr key={index} className={`lr-row${animating ? " fading" : ""}`}>
                                        ...
                                        <td className="lr-td">
                                            <div className="lr-date-range">
                                                <span className="lr-date">{formatDate(req.shiftChangeDate)}</span>
                                            </div>
                                        </td>
                                        <td className="lr-td">
                                            <span className="lr-reason">{req.preferredShift}</span>
                                        </td>
                                        <td className="lr-td">
